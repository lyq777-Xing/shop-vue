<template>
  ...
  <el-breadcrumb-item>收银员列表</el-breadcrumb-item>
</el-breadcrumb>
<el-card>
  ...
  <el-col :span="7"> 
   <el-input placeholder="请输入内容" v-model="input3" clearable @clear="getUserList">
    ...
    <el-col :span="4">
    <el-button class="btn1" @click="addDialogVisible=true">添加收银员</el-button>
    </el-col>
  </el-row>
...
export default {
    ...
    methods:{
     async getUserList(){
          const{data:res}=await this.$http.get('users/cashier')
                    console.log(res)
      ...
      async showEditDialog(id){
        const{data:res}=await this.$http.get("users/findCashierById?id="+id)
        if(res.meta.status === 403){
            this.$router.push("/welcome")
            return this.$message.error('权限不足，无法访问')
        }else if(res.meta.status!=200){
