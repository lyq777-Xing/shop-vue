...
<el-card>
  ...
  <el-col :span="7"> 
   <el-input placeholder="请输入内容" v-model="input3" clearable @clear="getUserList">
    ...
    <el-col :span="4">
    <el-button class="btn1" @click="addDialogVisible=true">添加用户</el-button>
    </el-col>
  </el-row>
...
deleteid:''
    ...
    methods:{
     async getUserList(){
          const{data:res}=await this.$http.get('users/simple')
...
this.$refs.addFormRef.resetFields()
    ...
    async showEditDialog(id){
      const{data:res}=await this.$http.get("users/findSimpleById?id="+id)
      if(res.meta.status === 403){
          this.$router.push("/welcome")
          return this.$message.error('权限不足，无法访问')
      }else if(res.meta.status!=200){
   ...
   async deleteUser(){
    ...
    async finById(){
     ...
     if(this.input3!=""&&this.select=='2'){
       const{data:res} =await this.$http.get('users/findSimpleById?id='+this.input3)
       console.log(res);
